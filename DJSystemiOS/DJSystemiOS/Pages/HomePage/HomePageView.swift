...
struct HomePageView: View {
    ...
    var body: some View {
        VStack {
            ...
            Button("検索する") {
                Task {
                    showingAlert = ((try! await controller?.searchRoom(byId: dataSource.searchQuery)) != nil)   
                }
            }
            .alert("ルームが見つかりませんでした", isPresented: $showingAlert) {
                Button("OK") {
                    // 了解ボタンが押された時の処理
                }
            } message: {
...
extension HomePageView {
    class DataSource: ObservableObject {
        @Published var searchQuery = "test"
